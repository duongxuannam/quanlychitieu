...
export default class TinTucThongBao extends Component {
    ...
    componentDidMount(){
        fetch('http://192.168.215.2:8080/API/TuVanTuyenSinh/LayTinTucTheoTrang.php?trang=' + this.state.page)
        .then(res=>res.json())
        .then(resjson=>{
            this.setState({
                mang:resjson
        ...
        return (
            <View style={{flex:1}}>
                ...
                <FlatList
                    ...
                    renderItem={({ item }) =>
                    <TouchableOpacity
                    onPress={()=>{this.props.navigation.navigate('ManHinh_NoiDung', {thamso:'aaa'})}}
                    >
                        <View style={ao.dong}>
                            <View style={ao.trai}>
                            <Image 
